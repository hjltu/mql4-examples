...
class Order {
    ...
    public:
        ...
        Order(double _gap, int _depth, double _profit, double _loss) {
            Print("Init...");
            gap = _gap;
            symbol = Symbol();
            ...
            if(order_price > current_price) {
                ...
                stop_loss = MathAbs(NormalizeDouble(order_price - medium*_loss, Digits));
                take_profit = MathAbs(NormalizeDouble(order_price + medium*_profit, Digits));
                }
            if(order_price < current_price) {
                order_type = OP_SELLSTOP;  
                stop_loss = order_price + medium*_loss;
...
void OnStart()
    ...
    Print("Start");
    RefreshRates();
    Order order(my_gap, my_depth, my_profit, my_loss);
